...
class StatisticsManager(object):
    ...
    def compute_route_statistics(self, route_index, duration_time_system=-1, duration_time_game=-1, failure_message=""):
        ...
        def set_infraction_message():
            ...
            # add timestamp and agent step information to the message
            message = event.get_message()
            event_dict = event.get_dict()
            if event_dict:
                timestamp = event_dict.get('timestamp')
                frame_number = event_dict.get('frame_number', 'N/A')
                agent_step = event_dict.get('agent_step', 'N/A')
                
                # if the message does not contain these information, add them
                if timestamp and '[' not in message:
                    if agent_step != 'N/A':
                        message += f" [Time: {timestamp:.3f}s, CARLA Frame: {frame_number}, Agent Step: {agent_step}]"
                    else:
                        message += f" [Time: {timestamp:.3f}s, CARLA Frame: {frame_number}]"
            route_record.infractions[infraction_name].append(message)
