...
class CollisionTest(Criterion):
    ...
    def _count_collisions(self, event):     # pylint: disable=too-many-return-statements
        ...
        # try to get the current agent step (if available)
        current_agent_step = None
        try:
            # get the current agent step from the blackboard
            current_agent_step = py_trees.blackboard.Blackboard().get('CurrentAgentStep')
        except:
            pass
        
        collision_event = TrafficEvent(event_type=actor_type, frame=GameTime.get_frame(), agent_step=current_agent_step)
        collision_event.set_dict({
            'other_actor': event.other_actor, 
            'location': actor_location,
            'timestamp': GameTime.get_time(),
            'frame_number': GameTime.get_frame(),
            'agent_step': current_agent_step
        })
        
        # build the message, including agent step information
        message_parts = [
            "Agent collided against object with type={} and id={} at (x={}, y={}, z={}) at time={:.3f}s".format(
        ...
        if current_agent_step is not None:
            message_parts.append(f"[CARLA Frame: {GameTime.get_frame()}, Agent Step: {current_agent_step}]")
        else:
            message_parts.append(f"[CARLA Frame: {GameTime.get_frame()}]")
        
        collision_event.set_message(" ".join(message_parts))
