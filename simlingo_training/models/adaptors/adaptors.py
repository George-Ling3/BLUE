...
from simlingo_training.utils.custom_types import DrivingExample, DeltaExample
...
class DeltaAdaptor(nn.Module):
    ...
    def forward(self, 
        ...
        try:
            delta_input = driving_example.delta_input
        except AttributeError:
            delta_input = driving_example
        
        b = delta_input.rough_waypoints.shape[0]
        inputs = None
    ...
    def compute_loss_delta(
        self, adaptor_features: Tensor, adaptor_logits: Tensor, _inputs: Dict[str, Tensor], example: DrivingExample,
        rough_waypoints: Tensor, rough_speed_waypoints: Tensor,
    ) -> Dict[str, Tuple[Tensor, Tensor]]:
        label = example.driving_label
        assert label is not None
        ...
        for i, input_type in enumerate(self.order):
            ...
            label = locals()[f'label_{input_type}']
            if input_type == 'route':
                label = label - rough_waypoints
            else:
                label = label - rough_speed_waypoints
...
class LanguageAdaptor(nn.Module):
    def __init__(self, language_model):
        super().__init__()
        self.embed_tokens = language_model.model.base_model.embed_tokens
...
class DeltaLanguageAdaptor(nn.Module):
    def __init__(self, language_model):
        super().__init__()
    ...
    def forward(self, example: DeltaExample, inference=False, **kwargs) -> Dict[str, Tensor]:
        try:
            delta_input = example.delta_input
        except AttributeError:
            delta_input = example
            
        b = delta_input.rough_waypoints.size(0)
        
        if inference:
            label = delta_input.delta_prompt_inference
        else:
            label = delta_input.delta_prompt
...
class DeltaAdaptorList(nn.Module):
    """
    Each adaptor is responsible for converting a driving example
    to a sequence of tokens and computing the loss on the token outputs.
    Adaptors are only used during training.
    ...
    def __init__(
        self,
        delta: Optional[DeltaAdaptor] = None,
        language: Optional[LanguageAdaptor] = None,
    ):
        super().__init__()
        self.driving = delta
    ...
    def forward(self, example: DeltaExample, **kwargs) -> Dict[str, Tensor]:
        """
        Construct input embeddings for the given driving example.
        """
    ...
    def compute_loss_delta(
        ...
        for key, adaptor in self.adaptors.items():
            ...
            if key == 'driving':
                losses = adaptor.compute_loss_delta(
                    adaptor_features, adaptor_logits, adaptor_input_dict, example,
                    rough_waypoints, rough_speed_waypoints,
                )
            else:
                losses = adaptor.compute_loss(adaptor_features, adaptor_logits, adaptor_input_dict, example)
            loss_dict.update(losses)
