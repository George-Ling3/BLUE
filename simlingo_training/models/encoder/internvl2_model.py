    ...
    def replace_placeholder_tokens(
        ...
        if inputs_embeds is None:
            ...
            if special_ids.size(0) > 0 and len(placeholder_values) > 0:
                ...
                first_occurrences = first_occurrences.transpose(0, 1) # 2D tensor of shape [num_special_ids, batch_size] containing indices where each special token first appears in each batch, if 0 then does not appear

                # get coords from label.placeholder_values with batch and special_id as key
                special_token_pos = first_occurrences.nonzero() # tensor of shape (N,2), each row [i,j] contains i=batch index containg the spectial token, j = special token index

                coords = []
                skip_token = []
                for id, (key_id, b_id) in enumerate(zip(special_token_pos[:, 1], special_token_pos[:, 0])):
                    if special_ids[key_id].item() in placeholder_values[b_id]:
                        skip_token.append(False)
                        coords.append(torch.tensor(placeholder_values[b_id][special_ids[key_id].item()], device=input_ids.device, dtype=wp_encoder_dtype))
                    else:
                        skip_token.append(True)
                        coords.append(torch.tensor([], device=input_ids.device, dtype=wp_encoder_dtype))
                # mask = torch.ones(special_token_pos.size(0), dtype=torch.bool, device=special_token_pos.device)
                # mask[skip_token_ids] = False
                # special_token_pos = special_token_pos[mask]
                # first_occurrences = first_occurrences[:, mask]
