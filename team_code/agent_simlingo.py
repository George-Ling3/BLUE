...
class LingoAgent(autonomous_agent.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None):
        ...
        self.blue_route_id = route_number.replace("bench2drive_", "")
        ...
        # Load config from the checkpoint Hydra folder.
        # self.config_load_path = Path(self.config_path) / 'simlingo' / '.hydra' / 'config.yaml'
        ...
        blue_mode = os.environ.get("BLUE_MODE", "")
        if blue_mode:
            if blue_mode != "trained_gate":
                raise ValueError("BLUE public evaluation only supports BLUE_MODE=trained_gate.")
            gate_ckpt = os.environ.get("BLUE_GATE_CKPT", None)
            if not gate_ckpt:
                raise ValueError("BLUE_GATE_CKPT must be set when BLUE_MODE=trained_gate.")
            gate_threshold = os.environ.get("BLUE_GATE_THRESHOLD", "0.66")
            cfg.model._target_ = "simlingo_training.models.driving_gate.DrivingModelGate"
            cfg.model.gate_mode = "trained_gate"
            cfg.model.gate_ckpt = gate_ckpt
            cfg.model.gate_kwargs = {"threshold": float(gate_threshold)}
            print(
                f"[BLUE] Gate enabled: mode=trained_gate, "
                f"checkpoint={gate_ckpt}, threshold={gate_threshold}"
            )
        ...
        # self.model.load_state_dict(torch.load(Path(self.config_path) / 'simlingo' / 'checkpoints' / 'epoch=013.ckpt' / 'pytorch_model.pt'))
        ...
        self.debug_save_path = str(self.save_path) + f'/{self.session}/iter_{self.iter}/{time.strftime("%Y_%m_%d_%H_%M_%S")}'
        ...
        self.blue_output_dir = os.environ.get('BLUE_OUTPUT_DIR', '')
        self.blue_gate_decision_path = pathlib.Path(self.save_path_metric) / "blue_gate_decisions.jsonl"
        self.blue_eval_summary_path = pathlib.Path(self.save_path_metric) / "blue_eval_summary.json"
        self.blue_global_gate_decision_path = None
        if self.blue_output_dir:
            pathlib.Path(self.blue_output_dir).mkdir(parents=True, exist_ok=True)
            self.blue_global_gate_decision_path = pathlib.Path(self.blue_output_dir) / "blue_gate_decisions.jsonl"
    ...
    def _init(self):
        ...
        self.metric_info = {}
        self.blue_gate_decision_log = []
        self.blue_gate_stats = {"generate": 0, "direct": 0}

    def _append_blue_gate_record(self, record):
        """Append one gate decision record to per-route and optional global JSONL logs."""
        for log_path in [self.blue_gate_decision_path, self.blue_global_gate_decision_path]:
            if log_path is None:
                continue
    ...
    def _write_blue_eval_summary(self):
        """Write the latest BLUE gate summary for the current route."""
        total = self.blue_gate_stats["generate"] + self.blue_gate_stats["direct"]
        generate_ratio = self.blue_gate_stats["generate"] / total if total > 0 else 0.0
        summary = {
            "route_id": self.blue_route_id,
            "gate_mode": os.environ.get("BLUE_MODE", "trained_gate"),
            "threshold": float(os.environ.get("BLUE_GATE_THRESHOLD", "0.66")),
            "stats": self.blue_gate_stats,
            "generate_ratio": round(generate_ratio, 4),
            "total_steps": total,
            "decision_log": self.blue_gate_decision_log,
        }
        with open(self.blue_eval_summary_path, "w", encoding="utf-8") as f:
    ...
    def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
        ...
        try: # Set the current agent step to the blackboard
            import py_trees
            py_trees.blackboard.Blackboard().set('CurrentAgentStep', self.step, overwrite=True)
        except:
            pass
        ...
        # Record BLUE gate decision for this step. Bench2Drive closed-loop
        # inference uses batch_size=1.
        if hasattr(self.model, 'gate_decisions') and self.model.gate_decisions:
            decision = int(self.model.gate_decisions[0])
            gate_score = None
            if hasattr(self.model, 'gate_scores') and self.model.gate_scores:
                gate_score = float(self.model.gate_scores[0])
            inference_time_ms = None
            if hasattr(self.model, 'gate_inference_time_ms') and self.model.gate_inference_time_ms:
                inference_time_ms = float(self.model.gate_inference_time_ms[0])
            metric_info_now = self.get_metric_info()
            lang_text = None
            if decision == 1 and language is not None and len(language) > 0:
                lang_text = language[0] if language[0] is not None else None
            record = {
                "route_id": self.blue_route_id,
                "step_id": int(self.step),
                "gate_score": gate_score,
                "gate_decision": decision,
                "threshold": float(os.environ.get("BLUE_GATE_THRESHOLD", "0.66")),
                "inference_time_ms": inference_time_ms,
                "speed": round(float(gt_velocity), 2),
                "acceleration": metric_info_now['acceleration'],
                "language": lang_text,
            }
            self.blue_gate_decision_log.append(record)
            self._append_blue_gate_record(record)
            if decision == 1:
                self.blue_gate_stats["generate"] += 1
            else:
                self.blue_gate_stats["direct"] += 1
        ...
        if DEBUG and self.step%5 == 0:
            ...
            if language is not None:
                ...
                # Direct-action gate decisions intentionally skip language output.
                lang_text = language[0] if language[0] is not None else "[Direct action - no language output]"
                lines = textwrap.wrap(f"Answer: {lang_text}", width=line_width)
                for idx, line in enumerate(lines):
                        draw.text((10, y_start + y_dist*(idx)), line, font=font, fill=(255, 255, 255, 255))
        ...
        if self.save_path_metric is not None and self.step % 1 == 0:
                # metric info
                outfile = open(os.path.join(self.save_path_metric, "metric_info.json"), 'w')
                json.dump(self.metric_info, outfile, indent=4)
                outfile.close()
                self._write_blue_eval_summary()
    ...
    def destroy(self, results=None):  # pylint: disable=locally-disabled, unused-argument
        ...
        """
        # Final save of BLUE gate decision summary.
        if hasattr(self, 'save_path_metric') and self.save_path_metric is not None and hasattr(self, 'blue_gate_stats'):
            self._write_blue_eval_summary()
            total = self.blue_gate_stats["generate"] + self.blue_gate_stats["direct"]
            generate_ratio = self.blue_gate_stats["generate"] / total if total > 0 else 0.0
            print(
                f"[BLUE] Route finished: generate={self.blue_gate_stats['generate']}, "
                f"direct={self.blue_gate_stats['direct']}, "
                f"generate_ratio={generate_ratio:.2%}, total_steps={total}"
            )
...
class ShadowLingoAgent(autonomous_agent.AutonomousAgent):
    """
        Main class that runs the agents with the run_step function
        """
    ...
    def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
        ...
        return pred_route, pred_speed_wps, language, self.DrivingInput
